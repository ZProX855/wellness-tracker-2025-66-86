...
const RoutineBuilder: React.FC<RoutineBuilderProps> = ({
  ...
  return <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card className="bg-white/80 backdrop-blur-sm border-wellness-softGreen/30 shadow-sm hover:shadow transition-all">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="routine-title" value={routineData.title} onChange={e => updateRoutineData({
            title: e.target.value
          })} placeholder="My Daily Routine" className="border-wellness-softGreen/30 focus:border-wellness-mediumGreen focus-visible:ring-wellness-mediumGreen/20" />
          </div>
          
          <div className="space-y-2">
            <Label htmlFor="routine-description">Description</Label>
            <Textarea id="routine-description" value={routineData.description} onChange={e => updateRoutineData({
            description: e.target.value
          })} placeholder="Personal daily tasks and habits" rows={3} className="border-wellness-softGreen/30 focus:border-wellness-mediumGreen focus-visible:ring-wellness-mediumGreen/20" />
          </div>
          
          <div className="space-y-2">
            <Label htmlFor="time-frame">Time Frame</Label>
            <Select value={routineData.timeFrame} onValueChange={value => updateRoutineData({
            timeFrame: value as 'daily' | 'weekly' | 'monthly'
          })}>
      ...
      <Card className="bg-white/80 backdrop-blur-sm border-wellness-softGreen/30 shadow-sm hover:shadow transition-all">
        ...
        <CardContent>
          {showNewTaskForm ? <form onSubmit={handleTaskSubmit} className="space-y-4 border rounded-md p-4 bg-white shadow-sm">
              <div className="space-y-2">
                <Label htmlFor="task-title">Task Title</Label>
                <Input id="task-title" value={newTask.title || ''} onChange={e => setNewTask({
              ...newTask,
              title: e.target.value
            })} placeholder="Exercise" required className="border-wellness-softGreen/30 focus:border-wellness-mediumGreen focus-visible:ring-wellness-mediumGreen/20" />
              </div>
              ...
              ...newTask,
              description: e.target.value
            })} placeholder="30 minutes of cardio" rows={2} className="border-wellness-softGreen/30 focus:border-wellness-mediumGreen focus-visible:ring-wellness-mediumGreen/20" />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button type="button" variant="outline" onClick={resetTaskForm} className="border-wellness-mediumGreen/30 hover:bg-wellness-softGreen/20 text-wellness-darkGreen">
                  Cancel
                </Button>
              ...
              {routineData.tasks.length === 0 ? <div className="text-center py-10 text-gray-500 flex flex-col items-center gap-3">
                  
                  
                </div> : routineData.tasks.map(task => <div key={task.id} className="flex items-center justify-between p-3 border border-wellness-softGreen/30 rounded-md hover:bg-wellness-softGreen/10 transition-colors">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <span className={`px-2 py-0.5 rounded-full text-xs ${task.priority === 'high' ? 'bg-red-100 text-red-800' : task.priority === 'medium' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'}`}>
                          {task.priority}
                        </span>
                    ...
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="icon" onClick={() => startEditTask(task)} className="hover:bg-wellness-softGreen/20 text-wellness-darkGreen">
