...
const ToolsSection: React.FC = () => {
  ...
  return <section className="px-4 sm:px-6 bg-white py-[60px]">
      <div className="max-w-6xl mx-auto py-0 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-wrap justify-center gap-3 sm:gap-4 opacity-0 animate-fade-in" style={{
        animationDelay: '300ms'
      }}>
          {categories.map((category, idx) => <button key={idx} onClick={() => setActiveCategory(idx)} className={cn("px-4 sm:px-6 py-2 sm:py-3 rounded-full text-sm font-medium transition-all duration-300", activeCategory === idx ? "bg-wellness-darkGreen text-white shadow-md" : "bg-wellness-softGreen/50 text-wellness-darkGreen hover:bg-wellness-softGreen")}>
              {category.name}
            </button>)}
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {categories[activeCategory].tools.map((tool, idx) => <div key={idx} className="opacity-0 animate-fade-in flex justify-center" style={{
          animationDelay: `${300 + idx * 100}ms`
        }}>
              <button onClick={() => navigate(tool.path)} className="w-[260px] h-[260px] sm:w-[280px] sm:h-[280px] rounded-full bg-wellness-softBeige hover:scale-[1.02] active:scale-[0.98] transition-all duration-500 ease-out-expo flex flex-col items-center justify-center text-center group p-4">
                <div className={`h-14 w-14 rounded-full bg-gradient-to-r ${tool.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-500`}>
                ...
                <p className="text-wellness-charcoal/70 text-sm mb-3 px-[28px]">{tool.description}</p>
