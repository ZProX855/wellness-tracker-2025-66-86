...
import { BarChart3, User, FileDown, Trash2, Flame, Apple, Camera, Heart, Moon, Calendar, Clock, ChefHat, Brain, MessageSquare, ArrowUpRight } from 'lucide-react';
  ...
  countLabel
    ...
    <div className="p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        {count !== undefined && <div className="bg-gray-100 rounded-full py-1 px-2.5 text-xs font-medium text-gray-700">
            {count} {countLabel || ''}
          </div>}
      </div>
      ...
      <Link to={path} className="inline-flex items-center text-xs font-medium text-wellness-darkGreen hover:text-wellness-mediumGreen group-hover:underline">
...
const Dashboard: React.FC = () => {
      ...
      try {
        if (user) {
          ...
          const {
            data: timetableData,
            error
          } = await supabase.from('timetable_history').select('*').eq('user_id', user.id).order('created_at', {
            ascending: false
          });
          if (error) {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      pdf.text('Wellness Tracker Report', 105, 15, {
        align: 'center'
      });
      ...
      pdf.text(`Generated for: ${user?.name}`, 105, 25, {
        align: 'center'
      });
      pdf.text(`Date: ${new Date().toLocaleDateString()}`, 105, 30, {
        align: 'center'
      });
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-gradient-to-b from-wellness-softBeige to-wellness-softGreen/30 flex items-center justify-center">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 mb-8 border border-wellness-softGreen/20 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              ...
              <div className="mt-4 md:mt-0 flex flex-col sm:flex-row gap-3">
                <button onClick={generatePDF} className="inline-flex items-center justify-center bg-wellness-softGreen hover:bg-wellness-mediumGreen text-wellness-darkGreen px-4 py-2 rounded-lg transition-colors disabled:opacity-70 text-sm" disabled={isGeneratingPdf}>
                  {isGeneratingPdf ? <>
                ...
                <Link to="/dashboard/profile" className="inline-flex items-center justify-center bg-white border border-wellness-softGreen/40 hover:bg-wellness-softGreen/20 text-wellness-darkGreen px-4 py-2 rounded-lg transition-colors text-sm">
                  <User className="h-4 w-4 mr-2" />
                  Profile
                </Link>
                
                <button onClick={handleResetProgress} className="inline-flex items-center justify-center bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2 rounded-lg transition-colors text-sm">
          ...
          <div className="flex items-center justify-center mb-8 overflow-x-auto md:overflow-visible scrollbar-hide">
            <div className="inline-flex bg-white/50 backdrop-blur-sm p-1 rounded-xl border border-wellness-softGreen/20 shadow-sm">
              <button onClick={() => setActiveTab('overview')} className={`py-2 px-4 text-sm font-medium rounded-lg transition-colors ${activeTab === 'overview' ? 'bg-wellness-darkGreen text-white shadow-sm' : 'text-wellness-darkGreen hover:bg-wellness-softGreen/20'}`}>
                <BarChart3 className="h-4 w-4 inline-block mr-1" />
                Overview
              </button>
              
              <button onClick={() => setActiveTab('bmi')} className={`py-2 px-4 text-sm font-medium rounded-lg transition-colors ${activeTab === 'bmi' ? 'bg-wellness-darkGreen text-white shadow-sm' : 'text-wellness-darkGreen hover:bg-wellness-softGreen/20'}`}>
                <Heart className="h-4 w-4 inline-block mr-1" />
                BMI
              </button>
              
              <button onClick={() => setActiveTab('food')} className={`py-2 px-4 text-sm font-medium rounded-lg transition-colors ${activeTab === 'food' ? 'bg-wellness-darkGreen text-white shadow-sm' : 'text-wellness-darkGreen hover:bg-wellness-softGreen/20'}`}>
                <Apple className="h-4 w-4 inline-block mr-1" />
                Food
              </button>
              
              <button onClick={() => setActiveTab('meals')} className={`py-2 px-4 text-sm font-medium rounded-lg transition-colors ${activeTab === 'meals' ? 'bg-wellness-darkGreen text-white shadow-sm' : 'text-wellness-darkGreen hover:bg-wellness-softGreen/20'}`}>
                <Camera className="h-4 w-4 inline-block mr-1" />
                Meals
              </button>
              
              <button onClick={() => setActiveTab('sleep')} className={`py-2 px-4 text-sm font-medium rounded-lg transition-colors ${activeTab === 'sleep' ? 'bg-wellness-darkGreen text-white shadow-sm' : 'text-wellness-darkGreen hover:bg-wellness-softGreen/20'}`}>
                <Moon className="h-4 w-4 inline-block mr-1" />
                Sleep
              </button>
              
              <button onClick={() => setActiveTab('timetable')} className={`py-2 px-4 text-sm font-medium rounded-lg transition-colors ${activeTab === 'timetable' ? 'bg-wellness-darkGreen text-white shadow-sm' : 'text-wellness-darkGreen hover:bg-wellness-softGreen/20'}`}>
          ...
          <div id="dashboard-content">
            {activeTab === 'overview' && <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  <StatCard icon={<Heart className="h-6 w-6" />} title="BMI Records" value={userData?.bmiHistory.length || 0} color="text-blue-600" bgColor="bg-blue-50" />
                  
                  <StatCard icon={<Apple className="h-6 w-6" />} title="Food Comparisons" value={userData?.foodComparisons.length || 0} color="text-green-600" bgColor="bg-green-50" />
                  
                  <StatCard icon={<Camera className="h-6 w-6" />} title="Meal Records" value={userData?.mealRecognitions.length || 0} color="text-amber-600" bgColor="bg-amber-50" />
                  
                  <StatCard icon={<Moon className="h-6 w-6" />} title="Sleep Entries" value={userData?.sleepData.length || 0} color="text-purple-600" bgColor="bg-purple-50" />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <TimetableHistoryPanel timetables={timetables} isLoading={timetablesLoading} />
                  
                  {userData?.bmiHistory && userData.bmiHistory.length > 0 ? <div className="bg-white bg-opacity-70 rounded-xl p-5 shadow-sm border border-wellness-softGreen/30 h-full">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-medium text-wellness-darkGreen">Recent BMI History</h3>
                        <Link to="/bmi-calculator" className="text-sm text-wellness-darkGreen hover:text-wellness-mediumGreen flex items-center">
                          New Check <ArrowUpRight className="ml-1 h-3 w-3" />
                        </Link>
                      </div>
                      
                      <div className="space-y-3">
                        {userData.bmiHistory.slice(0, 3).map((record: BMIRecord) => <div key={record.id} className="p-3 bg-wellness-softGreen/10 rounded-lg hover:bg-wellness-softGreen/20 transition-colors">
                            <div className="flex justify-between">
                              <div>
                                <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${record.category === 'Normal weight' ? 'bg-green-100 text-green-800' : record.category === 'Underweight' ? 'bg-blue-100 text-blue-800' : record.category === 'Overweight' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                                  {record.category}
                                </span>
                              </div>
                    ...
                    </div> : <div className="bg-white bg-opacity-70 rounded-xl p-5 shadow-sm border border-wellness-softGreen/30 h-full">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-medium text-wellness-darkGreen">Recent BMI History</h3>
                      </div>
                      <div className="text-center py-6">
                        ...
                        <Link to="/bmi-calculator" className="inline-flex items-center bg-wellness-darkGreen hover:bg-wellness-mediumGreen text-white px-3 py-1.5 rounded-lg text-sm transition-colors">
                          Calculate BMI
                        </Link>
                      </div>
                    </div>}
            ...
            {/* BMI History Tab */}
            {activeTab === 'bmi' && <div className="bg-white bg-opacity-70 backdrop-blur-sm rounded-xl p-6 border border-wellness-softGreen/30 shadow-sm">
                <h2 className="text-xl font-medium text-wellness-darkGreen mb-4">BMI History</h2>
                
                {userData?.bmiHistory && userData.bmiHistory.length > 0 ? <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-wellness-softGreen/30">
                      ...
                      <tbody className="bg-white bg-opacity-50 divide-y divide-wellness-softGreen/20">
                        {userData.bmiHistory.map((record: BMIRecord) => <tr key={record.id} className="hover:bg-wellness-softGreen/10">
                            ...
                            <td className="px-4 py-3 whitespace-nowrap text-sm">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${record.category === 'Normal weight' ? 'bg-green-100 text-green-800' : record.category === 'Underweight' ? 'bg-blue-100 text-blue-800' : record.category === 'Overweight' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>
                                {record.category}
                              </span>
                            </td>
                          </tr>)}
                  ...
                  </div> : <div className="bg-white bg-opacity-70 rounded-xl p-8 text-center border border-wellness-softGreen/30">
                    <Heart className="h-12 w-12 mx-auto text-wellness-darkGreen opacity-50 mb-3" />
                    ...
                    <Link to="/bmi-calculator" className="inline-flex items-center bg-wellness-darkGreen hover:bg-wellness-mediumGreen text-white px-4 py-2 rounded-lg transition-colors">
                      Calculate BMI
                    </Link>
                  </div>}
              </div>}
            ...
            {/* Food Comparison Tab */}
            {activeTab === 'food' && <div className="bg-white bg-opacity-70 backdrop-blur-sm rounded-xl p-6 border border-wellness-softGreen/30 shadow-sm">
                <h2 className="text-xl font-medium text-wellness-darkGreen mb-4">Food Comparison History</h2>
                
                {userData?.foodComparisons && userData.foodComparisons.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {userData.foodComparisons.map((comparison: FoodComparison) => <div key={comparison.id} className="bg-white bg-opacity-70 rounded-xl p-6 border border-wellness-softGreen/30 shadow-sm">
                    ...
                    <Link to="/food-compare" className="inline-flex items-center bg-wellness-darkGreen hover:bg-wellness-mediumGreen text-white px-4 py-2 rounded-lg transition-colors">
                      Compare Foods
                    </Link>
                  </div>}
              </div>}
            ...
            {activeTab === 'meals' && <div className="bg-white bg-opacity-70 backdrop-blur-sm rounded-xl p-6 border border-wellness-softGreen/30 shadow-sm">
                <h2 className="text-xl font-medium text-wellness-darkGreen mb-4">Meal Recognition History</h2>
                
                {userData?.mealRecognitions && userData.mealRecognitions.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {userData.mealRecognitions.map((meal: MealRecord) => <div key={meal.id} className="bg-white bg-opacity-70 rounded-xl overflow-hidden border border-wellness-softGreen/30 shadow-sm">
                        {meal.imageUrl && <div className="h-48 overflow-hidden">
                            <img src={meal.imageUrl} alt={meal.foodIdentified} className="w-full h-full object-cover" />
                          </div>}
                    ...
                    <Link to="/meal-recognition" className="inline-flex items-center bg-wellness-darkGreen hover:bg-wellness-mediumGreen text-white px-4 py-2 rounded-lg transition-colors">
                      Analyze a Meal
                    </Link>
                  </div>}
              </div>}
            ...
            {/* Sleep Tracking Tab */}
            {activeTab === 'sleep' && <div className="bg-white bg-opacity-70 backdrop-blur-sm rounded-xl p-6 border border-wellness-softGreen/30 shadow-sm">
                <h2 className="text-xl font-medium text-wellness-darkGreen mb-4">Sleep Tracking History</h2>
                
                {userData?.sleepData && userData.sleepData.length > 0 ? <div>
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-wellness-softGreen/30">
                        ...
                        <tbody className="bg-white bg-opacity-50 divide-y divide-wellness-softGreen/20">
                          {userData.sleepData.map((record: SleepRecord) => <tr key={record.id} className="hover:bg-wellness-softGreen/10">
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-wellness-darkGreen">
                                {new Date(record.date).toLocaleDateString()}
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap text-sm text-wellness-darkGreen">
                          ...
                          return `${hours}h ${minutes}m`;
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-sm">
                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${record.quality === 'Restful' ? 'bg-green-100 text-green-800' : record.quality === 'Good' ? 'bg-teal-100 text-teal-800' : record.quality === 'Average' ? 'bg-blue-100 text-blue-800' : record.quality === 'Light' ? 'bg-yellow-100 text-yellow-800' : record.quality === 'Disturbed' ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800'}`}>
                                  {record.quality}
                                </span>
                              </td>
                              <td className="px-4 py-3 text-sm text-wellness-darkGreen">
                                <div className="flex flex-wrap gap-1">
                                  {record.factors.map(factor => <span key={factor} className="px-2 py-0.5 bg-wellness-softGreen/30 rounded-full text-xs">
                                      {factor}
                                    </span>)}
                    ...
                    <Link to="/sleep-tracker" className="inline-flex items-center bg-wellness-darkGreen hover:bg-wellness-mediumGreen text-white px-4 py-2 rounded-lg transition-colors">
                      Track Your Sleep
                    </Link>
                  </div>}
              </div>}
            ...
            {activeTab === 'timetable' && <div className="bg-white bg-opacity-70 backdrop-blur-sm rounded-xl p-6 border border-wellness-softGreen/30 shadow-sm">
                <h2 className="text-xl font-medium text-wellness-darkGreen mb-4">Timetable History</h2>
                
                {timetables.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {timetables.map(timetable => <div key={timetable.id} className="bg-white bg-opacity-70 rounded-xl p-6 border border-wellness-softGreen/30 shadow-sm hover:border-wellness-mediumGreen/50 transition-colors">
                        <div className="flex justify-between items-start mb-3">
                          <h3 className="text-lg font-medium text-wellness-darkGreen">
                            {timetable.name}
                          </h3>
                        ...
                        {timetable.description && <p className="text-sm text-wellness-charcoal mb-4">{timetable.description}</p>}
                        
                        <div className="mb-4">
                          <div className="h-1.5 w-full bg-wellness-softGreen/20 rounded-full overflow-hidden">
                            <div className="h-full bg-wellness-mediumGreen rounded-full" style={{
                      width: `${Object.keys(timetable.schedule).length > 0 ? 100 : 0}%`
                    }}></div>
                          </div>
                        ...
                        <Link to={`/timetable-generator?id=${timetable.id}`} className="w-full inline-flex items-center justify-center bg-wellness-darkGreen hover:bg-wellness-mediumGreen text-white px-3 py-2 rounded-lg text-sm transition-colors">
                          View Timetable
                        </Link>
                      </div>)}
                  </div> : <div className="bg-white bg-opacity-70 rounded-xl p-8 text-center border border-wellness-softGreen/30">
                    ...
                    <Link to="/timetable-generator" className="inline-flex items-center bg-wellness-darkGreen hover:bg-wellness-mediumGreen text-white px-4 py-2 rounded-lg transition-colors">
                      Create Timetable
                    </Link>
                  </div>}
              </div>}
